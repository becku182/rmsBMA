...
x_names <- bma_list[[5]] # names of variables
K <- bma_list[[6]] + 1 # number of variables
alpha <- bma_list[[14]]
betas <- bma_list[[15]]

# Adding colnames and changing to dataframe
colnames(alpha) <- "Constant"
colnames(betas) <- x_names
alpha <- as.data.frame(alpha)
alpha <- data.frame(Const = as.numeric(alpha[, 1]))
...
if (kernel==1){
  histPlots[[1]]<-invisible(ggplot2::ggplot(alpha, ggplot2::aes(x = .data$Const)) +
                              ggplot2::geom_density(fill = "skyblue", alpha = 0.7) +
                              ggplot2::labs(
                                title = paste("Distribiution of constants"),
                                x = paste0("Constants"),
                                y = "Frequency") +
  ...
  names(histPlots)[[1]] <- "Const"
  for (i in 2:K){
    histPlots[[i]]<-invisible(ggplot2::ggplot(betas, ggplot2::aes(x = .data[[x_names[i-1]]])) +
                                ggplot2::geom_density(fill = "skyblue", alpha = 0.7) +
                                ggplot2::labs(
                                  title = paste("Distribiution of", x_names[i-1], "coefficients"),
                                  x = paste0("Coefficients on ",x_names[i-1]),
                                  y = "Frequency") +
                                ggplot2::theme_minimal(base_size = 12) +
                                ggplot2::theme(
                                  plot.title = ggplot2::element_text(size = 12, hjust = 0.5, face = "bold"),
                                  axis.title = ggplot2::element_text(face = "bold")))
    names(histPlots)[[i]] <- x_names[[i-1]]
  }
  ...
  if (BN==1){### Rules for bin width
    ...
    if (length(num)!=K){stop("num is missspecified: num should have K elements (number of regressors + 1)")}
    histPlots[[1]]<-invisible(ggplot2::ggplot(alpha, ggplot2::aes(x = .data$Const)) +

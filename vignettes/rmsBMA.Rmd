...
The object `modelSpace`, constructed according to the specification of the `model_space` function described above, is included in the package. The `model_space` function returns a list consisting of five components, comprising the full model space and additional elements required for subsequent use in the `bma` function. When the model space is sampled rather than enumerated, a sixth component holding the chain diagnostics is added; this is described in the next section.

## Sampling the model space: MC$^3$

Exhaustive enumeration becomes impossible well before the number of candidate regressors becomes large. With $K$ regressors and $M = K$ there are $2^K$ models: $1{,}024$ for the ten regressors in `Trade_data_small`, $131{,}072$ for the seventeen in `Trade_data`, and over a million once $K$ reaches twenty. Restricting the model space to $M < K$ postpones the problem but does not remove it, since $\sum_{j=0}^{M}\binom{K}{j}$ still grows quickly; with $K = 100$ and $M = 7$ the reduced space already contains roughly $1.6 \times 10^{10}$ models.

The standard remedy is to visit only the models that matter. Setting `mc3 = TRUE` replaces enumeration with Markov Chain Monte Carlo Model Composition [MC$^3$, @Madigan+1995; @Raftery+1997], a Metropolis--Hastings random walk over the model space. From the current model $\gamma$ a neighbour $\gamma'$ is proposed by adding or dropping a single regressor, chosen uniformly at random, and is accepted with probability

$$
\alpha(\gamma, \gamma') = \min\left\{1, \; \frac{p(y \mid \gamma')\, p(\gamma')}{p(y \mid \gamma)\, p(\gamma)}\right\}.
$$

Because the chain spends time in each model in proportion to its posterior probability, models with negligible posterior mass are simply never visited, and the cost of the exercise is governed by the number of models worth looking at rather than by the size of the space.

Two arguments control the chain. `draws` is the number of retained draws, and `burn` is the number of initial draws discarded, so the chain runs for `draws + burn` iterations in total. If `burn` is not supplied it defaults to `draws`, giving a fifty per cent burn-in.
...
Enumerating the same space takes about thirty seconds and estimates all $131{,}072$ models; the chain above takes roughly four seconds and finds the small number of models that carry essentially all of the posterior mass. From this point the object behaves exactly like an enumerated model space and is passed to `bma` in the usual way.
...
`model_space` reports several diagnostics when the chain finishes. The acceptance rate and the mean model size describe how the chain behaved; the number of distinct models visited indicates how concentrated the posterior is. The last line is the one that decides whether the run can be used at all:

```{r}
mc3Space[[6]]$cor_pmp
```

This is the correlation between two estimates of the same quantity. The first is analytic: for every visited model the posterior mass implied by its marginal likelihood and prior. The second is empirical: the frequency with which the chain actually visited that model. The two agree only once the chain has converged, so a value close to one indicates that the sampler has settled, while a lower value means the chain is too short and `draws` should be increased. Values below $0.99$ trigger a warning. The full set of diagnostics is available in the sixth element of the model space object, and is carried through to the sixteenth element of the `bma` object.
...
The `M` argument behaves exactly as it does under enumeration, so a reduced model space can be sampled as well as a full one. The restriction does, however, change the geometry of the problem in a way the sampler has to account for.

Under $|\gamma| \le M$ the number of neighbours of a model depends on its size. A model containing $r < M$ regressors admits $K - r$ additions and $r$ deletions, hence $K$ neighbours; a model already holding $M$ regressors cannot add anything and therefore has only $M$. The proposal is no longer symmetric, and the acceptance probability picks up a Hastings factor:
...
Omitting this factor produces no error and no warning. It quietly changes the distribution the chain converges to, from $p(\gamma \mid y)$ to one proportional to $p(\gamma \mid y)\,|\mathrm{nbd}(\gamma)|$. Since the models at the boundary are the ones with the *smaller* neighbourhood, they are the ones that lose mass, and inference is pulled towards models smaller than $M$. The correction is applied automatically. When $M = K$ every model has exactly $K$ neighbours, the factor equals one, and the sampler reduces to the textbook symmetric random walk.

### Verifying the sampler against enumeration

Because `Trade_data_small` contains only ten regressors, its model space is small enough to enumerate exactly, which makes it possible to check the sampler against a known answer rather than take it on trust. Consider first the full space of $1{,}024$ models.
...
The posterior inclusion probabilities agree to several decimal places, even though the chain visited only a small fraction of the $1{,}024$ available models. The models it skipped are precisely those whose posterior probability is too small to affect any of the reported quantities.

The same exercise can be repeated on a reduced model space, which is where the proposal correction matters. Restricting to at most five regressors leaves $638$ models, still small enough to enumerate.
...
The effect of the correction is clearest in the distribution of posterior mass across model sizes, since it is the models at the boundary $r = M$ whose weight is at stake.
...
The sampled mass at each model size tracks the enumerated mass closely. It is worth seeing what would happen without the correction, which can be computed directly from the enumerated answer: the uncorrected chain would converge to a distribution proportional to $p(\gamma \mid y)\,|\mathrm{nbd}(\gamma)|$, so the mass at the boundary would be
...
a shift of several percentage points away from the boundary and towards smaller models, which would be invisible in any output the package produces.

### When the chain barely moves

At the boundary only deletions are proposed, so moving from one model of size $M$ to a different model of the same size requires first accepting a smaller and typically worse model. When the posterior is sharply concentrated on a single specification of size $M$ --- which is precisely the situation reduced model space BMA is designed for --- the chain can accept very few proposals and visit very few models.

The convergence correlation is of no help here, because the handful of models the chain did visit agree perfectly among themselves and the correlation is close to one. `model_space` therefore issues a separate warning whenever the acceptance rate falls below two per cent. If that warning appears, the visited set should be treated as a possibly incomplete picture of the model space, and the result checked against a longer chain or, where feasible, against enumeration.

### Extreme Bounds Analysis is not available for a sampled model space

Extreme bounds are defined by the smallest and largest coefficient estimates across *all* models, and a sampler never visits the extremes of a distribution it is exploring by posterior mass. Bounds computed from a visited subset are therefore systematically too narrow, which makes a regressor appear more robust than it is. Since this failure is silent and always points in the same direction, EBA is withheld entirely rather than reported with a caveat: the third element of the `bma` object is `NULL` and a message explains why.

```{r}
is.null(mc3_results[[3]])
```

Dilution priors, by contrast, remain available. Model priors are applied after the chain has run, by reweighting the visited models, so no re-estimation is required. A warning is issued because the chain explored the space under a uniform model prior rather than under the dilution prior itself; the reweighting is dependable when dilution is mild, and less so when it strongly downweights the region the chain explored.

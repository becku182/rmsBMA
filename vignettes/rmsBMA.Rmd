...
Setting `mc3 = TRUE` replaces it with Markov Chain Monte Carlo Model Composition [MC$^3$, @Madigan+1995; @Raftery+1997], a Metropolis--Hastings random walk over the model space. A neighbor $\gamma'$ of the current model $\gamma$ is proposed by adding or dropping a single regressor and accepted with probability
...
where $|\mathrm{nbd}(\cdot)|$ counts a model's neighbors. The chain visits each model in proportion to its posterior probability, so models carrying negligible mass are never visited at all and the cost depends on how many models matter rather than on how many exist.
...
Under $|\gamma| \le M$ the neighborhood is not of constant size. A model of size $r$ admits $K - r$ additions and $r$ deletions while $r < M$, but one already holding $M$ regressors can only delete:
...
and since boundary models carry the smaller neighborhood, they are the ones that lose mass: inference drifts towards models smaller than $M$. The correction is applied automatically, and equals one when $M = K$.
...
To view the results for only the best models, the user can use the `top` parameter. Both `model_pmp` and `model_sizes` also take a `type` argument: `"line"`, the default, draws the prior and posterior as lines, while `"histogram"` draws them as side-by-side bars. The bars make individual models easier to tell apart once `top` is small, and the lines stay readable when it is not; the numbers behind the two are the same.
...
The same `type` argument applies here. Model size is discrete, so `"histogram"` puts a bar at each integer size, which makes the mass at any one of them easier to read; the line form is easier to follow when the model space is large.

...
Under $|\gamma| \le M$ the neighbourhood is not of constant size. A model with $r < M$ regressors admits $K - r$ additions and $r$ deletions, hence $K$ neighbours, while one already holding $M$ can only delete and so has $M$. The proposal is therefore asymmetric, which is what the Hastings factor in the acceptance probability above corrects. Omitting that factor raises no error but changes the target to something proportional to $p(\gamma \mid y)\,|\mathrm{nbd}(\gamma)|$: boundary models carry the smaller neighbourhood, so they lose mass and inference drifts towards models smaller than $M$. The correction is applied automatically, and equals one when $M = K$.

...
Two restrictions apply to MC$^3$ in the current version of the package.

**MC$^3$ is available only for the full model space.** Supplying `M` smaller than the total number of regressors together with `mc3 = TRUE` produces an error rather than a silently altered model space. The reason is that the proposal used above is symmetric only when $M = K$: every model then has exactly $K$ single-flip neighbours, so $q(\gamma' \mid \gamma) = q(\gamma \mid \gamma') = 1/K$ and the Hastings ratio equals one. Once $M < K$ this ceases to hold, because a model already containing $M$ regressors can only drop a variable and therefore has $M$ neighbours, while a smaller model has $K$. The acceptance probability then requires the correction
...
and omitting it would bias the posterior towards models of size $M$ without producing any visible symptom. Reduced model space sampling is therefore deferred until that correction is implemented and verified.
